...
function findIndexByProperty(arr, name) {
  ...
  let n = arr.findIndex(values => values = name )
  return n
...
function findLastEvenNumber(arr) {
  ...
  let p = arr.findLast(arr => arr % 2 ==0)
  return p
...
function getPositiveArrayValues(arr) {
  ...
  const arrayVal = Array.from(valIter);
  const positiveVal = arrayVal.filter((value) => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let mas = arr.flat()
  let m2 = mas.map(x => x**2)
  let result = m2.filter( x => x > threshold  )
  return result
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let arrn = arr.slice(n)
  let result = arrn.reduceRight((accumulator, curentValue) => accumulator + curentValue)
  arrn.forEach(element => console.log(element));
  return result
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
const arrN = arr.flatMap((num) => Array(n).fill(num));
  return arrN;
...
function calculateTotalPrice(arr) {
  ...
  let sum = arr.reduce((acc, arr) => {return acc + arr.price},0);
  return sum
...
function findLastNegativeNumberIndex(arr) {
  ...
  let found = (element) => element <0;
  return arr.findLastIndex(found)
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
